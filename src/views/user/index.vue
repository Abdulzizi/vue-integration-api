                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <BModal v-model="isOpenForm" size="lg" id="modal-standard" :title="modalTitle"
                                    ...
                                    @cancel="isOpenForm = false; userStore.resetState(); getUsers()"
                                    @close="isOpenForm = false; userStore.resetState(); getUsers()">
                                    <BRow>
                                        <BCol cols="12">
                                        ...
                                        <BCol cols="12" class="mt-4">
                                            <BForm class="form-horizontal" role="form">
                                                ...
                                                <BRow class="mb-3">
                                                    <label for="form-role" class="col-md-2 col-form-label">Role</label>
                                                    <BCol md="10">
                                                        <select id="form-role" class="form-select" :class="{
                                                            'is-invalid': !!(errorList && errorList.m_user_roles_id),
                                                        }" v-model="formModel.m_user_roles_id">
                                                            <option value="" disabled>Pilih Role</option>
                                                            <option v-for="role in roles" :key="role.id"
                                                                :value="role.id">{{ role.name }}</option>
                                                        </select>

                                                        <template v-if="!!(errorList && errorList.m_user_roles_id)">
                                                            <div class="invalid-feedback"
                                                                v-for="(err, index) in errorList.m_user_roles_id"
                                                                :key="index">
                                                                <span>{{ err }}</span>
                                                            </div>
                                                        </template>
